...
import {
  ArrowLeft, Clock, Tag, Calendar,
  Copy, Check, BookOpen, ArrowRight, Sparkles
} from 'lucide-react';
import Link from 'next/link';
import Navbar from '@/components/Navbar';
import Footer from '@/components/Footer';
import { getAllBlogs, type BlogPost } from '@/data/blogs';
...
function ShareButton() {
  ...
  return (
    <button
      onClick={copy}
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-muted border border-[rgba(0,0,0,0.04)] text-[0.68rem] font-bold text-on-surface-muted hover:text-primary hover:border-primary/15 transition-all cursor-pointer"
    >
...
export default function BlogReaderClient({ post }: { post: BlogPost }) {
  ...
  const allPosts = getAllBlogs().filter(p => p.slug !== post.slug);
              ...
              <div className="mt-8 p-5 sm:p-6 rounded-xl bg-on-surface text-white relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col sm:flex-row items-center gap-4">
                  <div className="flex-1 text-center sm:text-left">
                    ...
                    <p className="text-white/50 text-[0.78rem] font-medium">Let's build the right solution for your business with Tarun & TheUniPick.</p>
                  </div>
              ...
              <div className="mt-8 p-4 rounded-xl bg-surface-muted border border-[rgba(0,0,0,0.04)]">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-[0.6rem] text-on-surface-muted font-medium">Principal Full-Stack Engineer</div>
                  </div>
                </div>
              </div>
            </aside>
