...
export async function generateMetadata({ params }: { params: Promise<{ service: string }> | { service: string } }): Promise<Metadata> {
  const resolvedParams = await Promise.resolve(params);
  const service = resolvedParams?.service ? getServiceBySlug(resolvedParams.service) : undefined;
...
export default async function ServicePage({ params }: { params: Promise<{ service: string }> | { service: string } }) {
  const resolvedParams = await Promise.resolve(params);
  const service = resolvedParams?.service ? getServiceBySlug(resolvedParams.service) : undefined;
